...
class PackagingGroup extends React.Component  {
    //(props.parent.catalog_tittle===p.catalog_tittle)
    render() {
        const allPackage = ["кг", "кор", "шт"];

        const packaging = _try(() => this.props.fish.packaging, null)

        return (<Fragment>

            <select id="add_fish_packaging">
                {
                    allPackage.map((p, index) => (
                        <option key={index} selected={(packaging === p)} value={p}>{p}</option>))
                }
            </select>
        ...
        return ( <div className={css(SeafoodItemStyle.cnt)}>
            ...
            <div className={css(SeafoodItemStyle.flexContainer)}>
                ...
                <div className={css(SeafoodItemStyle.infoContainer, SeafoodItemStyle.width30P)}>
                    ...
                    <div>Фасовка
                        <PackagingGroup  ref={packagingGroupRef} fish={item}/>
                    </div>
