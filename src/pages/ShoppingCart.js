  ...
  return (
    <Switch>
      <Route
        ...
        path="/shopping_cart"
        params={props}
        component={connect(mapStateToProps)(Basket)}
      />
      <Route
        path="/shopping_cart/:num"
        params={props}
        component={BasketConfirmed}
      />
    </Switch>
  );
};
    ...
    <div className={css(ShoppingCartStyle.cnt)}>
      ...
      <div className={css(ShoppingCartStyle.productCnt)}>
        ...
        <div
          ...
          className={css(ShoppingCartStyle.h1)}
        >
          Доставка
        </div>
        ...
        <div className={css(ShoppingCartStyle.coupon)}>
          ...
          <div>
            <button
              onClick={() => {}}
              style={{ width: '144px' }}
              className={css(AppStyle.buttonGrey)}
            >
              Применить
            </button>
          </div>
        ...
        <div className={css(ShoppingCartStyle.totalText)}>
          ...
          <span className={css(ShoppingCartStyle.price)}>
            <nobr>
              {(
                props.seafoodShoppingCart.totalCost +
                props.placeOfDelivery.price
              ).toFixed(0)}{' '}
              руб
            </nobr>
          </span>
              ...
              onClick={(event) => {
                if (
                  ...
                  for (let el in props.seafoodShoppingCart.allFish) {
                    ...
                    msg += `-${i}) ${fish.name} ${fish.count} ${
                      fish.packaging
                    } по ${fish.price} руб  на сумму ${fish.cost}руб.\n`;
                    i++;
                  }
                  msg += `Доставить ${
                  ...
                  $.ajax({
                    url: `http://telegram.fishbazar63.ru/bot274564744:AAEGwFztcyAHcXcuNNR6ZzwiXoY4eY-nXCM/sendMessage?text=<code>${encodeURI(
                      msg,
                  ...
                  $.ajax({
                    url: `http://telegram.fishbazar63.ru/bot274564744:AAEGwFztcyAHcXcuNNR6ZzwiXoY4eY-nXCM/sendMessage?text=<code>${encodeURI(
                      msg,
                    )}</code>&parse_mode=HTML&chat_id=92139864`,
                    dataType: 'JSONP',
                    jsonpCallback: 'callback',
                    type: 'GET',
                    success: function() {},
    ...
    <div className={css(ShoppingCartStyle.cnt)}>
      ...
      <div className={css(ShoppingCartStyle.productCnt)}>
        <div className={css(ShoppingCartStyle.deliveryInfo)}>
          ...
          <div
            style={{ height: '53px' }}
            className={css(ShoppingCartStyle.mobileShow)}
          />
          <div>
            <div
              className={css(ShoppingCartStyle.h1)}
            ...
            <div className={css(ShoppingCartStyle.confirmedText)}>
              Ваш заказ в обработке, мы перезвоним вам в течение часа, чтобы
              узнать когда отправить курьера с морепродуктами
            </div>
          </div>
        </div>
      </div>
