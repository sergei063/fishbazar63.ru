    ...
    render() {
        ...
        return (
            <div>
                <Link
                    id="header_pouch"
                    className={css(lenght === 0 ? Style.displayNone : Style.pouchA)}
                    to="/shopping_cart">
                    <span className={css(Style.pouch_a_span)}>{lenght}</span>
                </Link>
            </div>
        )
    }
...
const Style = StyleSheet.create({
    ...
    pouchA: {
        ...
        'only screen and (-webkit-min-device-pixel-ratio: 2), only screen and (min--moz-device-pixel-ratio: 2), only screen and (-o-min-device-pixel-ratio: 2/1), only screen and (min-device-pixel-ratio: 2), only screen and (min-resolution: 192dpi), only screen and (min-resolution: 2dppx)': {
            background: `#ffffff  -webkit-image-set(url(${require('../../img/bucket/normal.png')}) 1x, url(${require('../../img/bucket/normal@2x.png')}) 2x, url(${require('../../img/bucket/normal@3x.png')}) 3x)  no-repeat 50% 19px`,
