...
export const MenuBlogFilter = (props) => {
  const blogFilter = _try(() => props.history.location.state.blogFilter, '');
  return (
    <div className={css(MenuBlogFilterStyle.cnt_filter)}>
      {Recipes.getGroup().map((group, index) => (
        <div
          className={css(
            MenuBlogStyle.el_filter,
            blogFilter === group.catalog_tittle
              ? MenuBlogStyle.el_filter_selected
              : null,
          )}
          key={index.toString()}
          onClick={() => {
            props.history.push({
              pathname: '/blog',
              state: { blogFilter: group.catalog_tittle },
              notScrollToTop: true,
            });
          }}
        >
...
const MenuBlog = (props) => {
  const fishForRecipe = findFishForRecipe(props.fish_for_recipe);
    ...
    <div className={css(MenuBlogStyle.menu)}>
      ...
      <div className={css(MenuBlogStyle.menu_about)}>
        <div style={{ width: '207px' }}>
          Блог о том, как готовить морепродукты
        </div>
        <div style={{ height: '123px' }}>
          <img
            src={require('../../../img/design/blog/layer/l.png')}
...
const MenuBlogStyle = StyleSheet.create({
  ...
  socialNetworkWatchDiv: {
    ...
    backgroundImage:
      `url(${require('../../../img/design/delivery_small/small.png')})`,
